...
## 🏗️ Virtual Lab Setup (No Code Required)

### Your Research Question
**"Find independent Alzheimer's disease signals in the APOE region beyond E2/E3/E4, while addressing methodological challenges of strong E4 effects and LD reference panel mismatches"**

---

## 👥 Agent Definitions

Copy these agent prompts and use them in separate Claude conversations:
...
- Alternative conditioning strategies (LDSC, stratified analysis)
- Diagnosing spurious associations from LD mismatches

CURRENT PROJECT: APOE region analysis where conditioning on E4 variant creates false independent signals due to LD reference panel issues.
...
5. Always consider population stratification effects
...
- Multi-signal colocalization (COLOC-SuSiE, eCAVIAR)
...
CURRENT PROJECT: Multiple xQTL datasets show colocalization with APOE GWAS, but these may be LD artifacts from the dominant E4 effect.
...
SPECIFIC PROBLEM: 
When I condition my APOE GWAS analysis on E4 variants (rs429358), I get spurious independent signals because my LD reference panel doesn't perfectly match my study population. The E4 effect is so strong that even small LD mismatches create false positives.
...
- Individual-level genotype data for subset
...
SPECIFIC PROBLEM:
I have eQTL, pQTL, and sQTL data showing multiple colocalizations with my APOE GWAS signals. However, I suspect many are false positives due to LD artifacts from the dominant E4 effect creating spurious molecular associations.
...
- Cross-tissue consistency is variable
...
SPECIFIC PROBLEM:
Standard fine-mapping (SuSiE, FINEMAP) fails in APOE region because the E4 effect is so strong it overwhelms the statistical models. I need approaches that can identify independent signals despite this dominant confounder.

MY CURRENT RESULTS:
- SuSiE identifies E4 variants in first component, then fails  
- FINEMAP converges poorly, unstable credible sets
- Different methods give inconsistent results
- Posterior probabilities unreliable due to model misspecification
...
CONTEXT:
After addressing methodological issues, I may identify candidate independent variants beyond E2/E3/E4. I need to evaluate their biological plausibility and prioritize for validation.

CANDIDATE SCENARIOS:
1. Regulatory variants affecting APOE expression levels
2. Variants in nearby genes (TOMM40, APOC1, PVRL2)  
